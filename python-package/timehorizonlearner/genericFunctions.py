...
def _indexDataFrame(df, list_unique_periods, varId, varPeriod, afterNPeriod):
    ...
    if afterNPeriod > len(list_unique_periods):
        print(
            "Warning: indexDataFrame: afterNPeriod shoould be strictly lower than the number of unique elements in column " + str(
                varPeriod))
        return df
...
def scaler(result):
    ...
    for i in range(result2.shape[1]):
        ...
        for j in range(1, result2.shape[2]):
            result2[:, i, j] = (result2[:, i, j] - np.mean(result2[:, i, j])) / np.sqrt(
                np.var(result2[:, i, j])) * np.sqrt(storeVar) + storeMean
