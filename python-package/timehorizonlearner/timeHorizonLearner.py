...
def xrange(x):
    """Dirty fix due to compatibility issue between python2 and python3"""
    return iter(range(x))
...
class timeHorizonLearner(tree.DecisionTreeClassifier):
    ...
    def complete_transitions(self, X, verbose=True):
        ...
        if self.__usable_idperiod is not None:
            ...
            usable_lines = np.where(result[self.varPeriod] >= result["firstUsablePeriod"])[
                0]  # [0] to handle specific shape of np.where results
        else:
        ...
        if verbose is True:
            print("timeHorizonLearner.complete_transitions: transition has: " + str(
                sum(sum(self.__transitions))) + " elements in it.")
        ...
        local_used_idperiod = group_max(x_reduced[self.varId].values, x_reduced[self.varPeriod].values, self.varId,
                                        "lastUsedPeriod")
        # merge it
        ...
        local_usable_idperiod = group_min(result[self.varId].values, result[self.varPeriod].values, varGroup=self.varId,
                                          varData="firstUsablePeriod")
        ...
        if self.__usable_idperiod is not None:
            ...
            local_usable_idperiod = group_max(local_usable_idperiod[self.varId].values,
                                              local_usable_idperiod["firstUsablePeriod"].values, self.varId,
                                              "firstUsablePeriod")
        # Integrate it 
    ...
    def evaluator(self, X, y, verbose=True):
        ...
        if self.__oob_score == 0.5:
            print(
                "timeHorizonLearner.evaluator: WARNING: This the case where in next period every one has the same class, we can't evaluate ourself")
        if verbose is True:
    ...
    def predict_proba(self, X_deploy, jumps=1, verbose=True):
        ...
        if isinstance(X_deploy, np.ndarray):
            if X_deploy.shape[1] == len(self.features_):
                ...
                raise ValueError(
                    "timeHorizonLearner.predict_proba: error you gave a numpy.ndarray but not with the same number of columns as in train. You can try to feed a pandas.core.frame.DataFrame.")
    ...
    def __get_transitions_prob(self, verbose):
        ...
        if verbose is True:
            ...
            n_poor_transitions = np.sum(transitions_prob < min_n_per_transition, axis=(0, 1)) - np.sum(
                transitions_prob == 0, axis=(0, 1))
            print("timeHorizonLearner.__get_transitions_prob: there are " + str(
                n_poor_transitions) + " poorly estimated transition (" + str(
                round(float(n_poor_transitions) / n_transitions * 100)) + "%), i delete them.")
            n_transitions = sum(sum(transitions_prob))
        transitions_prob[transitions_prob < min_n_per_transition] = 0.
        if verbose is True:
            print("timeHorizonLearner.__get_transitions_prob: they represented: " + str(
                n_transitions - sum(sum(transitions_prob))) + " transtions (" + str(
                round(float(n_transitions - sum(sum(transitions_prob))) / sum(sum(self.__transitions)) * 100)) + "%).")
    ...
    def __build_transitions(self, start_states, end_states, verbose=True):
        ...
        if verbose == "debug":
            print(
                "Debug: it is " + str(time.time()) + " Identify this starting states: " + str(np.unique(start_states)))
        ...
        if self.__transitions is None:
            ...
            if verbose is True:  # == "debug":
                ...
                print("timeHorizonLearner.__build_transitions: we added: " + str(
                    n_transition_aft - n_transition_bef) + " transitions.")
    ...
    def __build_states_index(self, verbose=True):
        ...
        # To-do: degorifier
        states = np.where(self.clf.tree_.children_left == -1)[
            0].tolist()  # Filter to get only leafs (nodes that haven't a left child)
        ## Build indexOfStates
        for i in range(len(states)):
            self.__states_index[states[i]] = i

        if verbose is True:
            print("timeHorizonLearner.__buildListInStates: there are " + str(
                len(states)) + " final leafs in the classifier.")
    ...
    def __build_states_prob(self, verbose=True):
        ...
        if len(self.classes_) != len(self.clf.classes_.tolist()):
            ...
            if verbose == "debug":
                print("Debug: timeHorizonLearner.__build_states_prob: list of missing classes indexes: " + str(
                    missing_classes_))
    ...
    def __build_features_(self, X):
        ...
        for var in X.columns:
            if not var in [self.varId, self.varPeriod, self.varNextPeriod] + ['idNextPeriod' + str(i) for i in
                                                                              range(100)]:
                self.features_.append(var)
        # sample features_
        self.features_ = [self.features_[i] for i in
                          sorted(random.sample(xrange(len(self.features_)), int(math.sqrt(len(self.features_)))))]
    ...
    def get_n_transitions(self):
        """Control function"""
        print("timeHorizonLearner.complete_transitions: transition has: " + str(
            np.sum(self.__transitions, axis=(0, 1))) + " elements in it.")
    ...
    def infinite_distrib(self, verbose=True):
        ...
        return np.dot(self.__transitions.sum(axis=1) / sum(sum(self.__transitions)),
                      np.dot(converged_transitions_prob, self.__states_prob))
